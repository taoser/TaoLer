		...
		<div class="layui-col-md8">
			<div class="fly-panel" style="margin-bottom: 0;">
				<div class="layui-tab layui-tab-card" style="margin-top: 0px;">
					...
					<li <?php if($type == 'all'): ?> class="layui-this" <?php endif; ?>><a href="<?php echo htmlentities((string) app('request')->domain()); ?><?php echo url('cate_type',['ename' => app('request')->param('ename'),'type' => 'all']); ?> "  ><?php echo lang('all'); ?></a></li>
					<li <?php if($type == 'jie'): ?> class="layui-this" <?php endif; ?>><a href="<?php echo htmlentities((string) app('request')->domain()); ?><?php echo url('cate_type',['ename' => app('request')->param('ename'),'type' => 'jie']); ?> "><?php echo lang('end'); ?></a></li>
					<li <?php if($type == 'wait'): ?> class="layui-this" <?php endif; ?>><a href="<?php echo htmlentities((string) app('request')->domain()); ?><?php echo url('cate_type',['ename' => app('request')->param('ename'),'type' => 'wait',]); ?>"><?php echo lang('未解决'); ?></a>	</li>
					</ul>
				</div>
				<ul class="question-list">
					<?php if(is_array($artList['data']) || $artList['data'] instanceof \think\Collection || $artList['data'] instanceof \think\Paginator): $i = 0; $__LIST__ = $artList['data'];if( count($__LIST__)==0 ) : echo "还没有内容" ;else: foreach($__LIST__ as $key=>$article): $mod = ($i % 2 );++$i;?>
					<li>
						<?php if(($article['jie'] == 1)): ?>
						...
						<div class="que-sta-view layui-hide-xs">
							<span><?php echo htmlentities((string) $article['pv']); ?></span>
							<span >阅读</span>
						</div>
						<div class="que-body">
							<h2><a href="<?php echo htmlentities((string) $article['url']); ?>"  style="color:<?php echo !empty($article['title_color']) ? htmlentities((string) $article['title_color']) : "#333"; ?>;"><?php echo htmlentities((string) $article['title']); ?></a></h2>
							<div class="que-user-info">
								<a href="<?php echo url("user/home",["id"=>$article['user']['id']])->domain(true); ?>" class="que-avatar">
								...
								<?php echo htmlentities((string) date('Y-m-d',!is_numeric($article['create_time'])? strtotime($article['create_time']) : $article['create_time'])); if(!empty($article['has_img'])) echo '<span><i class="layui-icon layui-icon-picture" style="color: #5FB878;"></i></span>'; if(!empty($article['has_video'])) echo '<span><i class="layui-icon layui-icon-play" style="color: #FF5722;"></i></span>'; if(!empty($article['has_audio'])) echo '<span><i class="layui-icon layui-icon-speaker" style="color: #000000;"></i></span>'; if(!empty($article['read_type'])) echo '<span><i class="layui-icon layui-icon-password" style="color: #FF5722;"></i></span>'; if(!empty($article['upzip'])) echo '<span><i class="layui-icon layui-icon-file-b" style="color: #009688;" title="附件"></i></span>'; ?>
								<span ><i class="iconfont icon-pinglun1" title="回答"></i> <?php echo htmlentities((string) $article['comments_count']); ?></span>
							</div>
						</div>
					</li>
