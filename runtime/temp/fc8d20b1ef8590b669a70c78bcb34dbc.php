...
<head>
  ...
  <meta name="description" content="<?php echo htmlentities((string) $article['title']); ?>,<?php echo isset($article['description']) ? htmlentities((string) $article['description']) : ''; ?>" />
  ...
  <meta property="og:description" content="<?php echo htmlentities((string) $article['title']); ?>,<?php echo isset($article['description']) ? htmlentities((string) $article['description']) : ''; ?>" />
    ...
    <div class="layui-row layui-col-space15">
		<div class="layui-col-md8 content detail">
			<section class="fly-panel detail-box">
				...
				<div class="user-questions">
					<a class="user-avatar" href="<?php echo url("user/home",["id"=>$article['user']['id']])->domain(true); ?>">
					...
					<span class="user-post-time" data="<?php echo htmlentities((string) $article['create_time']); ?>" style="padding-top: 5px"><?php echo htmlentities((string) date("Y-h-d H:i",!is_numeric($article['update_time'])? strtotime($article['update_time']) : $article['update_time'])); ?></span>
					<i class="iconfont" title="<?php echo lang('reply'); ?>">&#xe60c;</i>
					<?php echo htmlentities((string) $article->comments_count); ?>
					<i class="iconfont" title="浏览">&#xe60b;</i>
					<?php echo htmlentities((string) $pv); if(($article['jie'] == 0)): ?>
					<span class="layui-btn layui-btn-xs" style="background-color: #ff5722"><?php echo lang('no finished'); ?></span>
					<?php else: ?>
					<span class="layui-btn layui-btn-xs" style="background-color: #009688"><?php echo lang('finished'); ?></span>
					<?php endif; ?>
					<span id="LAY_jieAdmin" data-id="<?php echo htmlentities((string) $article['id']); ?>"></span>
				</div>
				...
				<div style="margin-top: 20px">本文链接：
					<a href="<?php echo htmlentities((string) app('request')->domain()); ?><?php echo htmlentities((string) app('request')->url()); ?>">
						<?php echo htmlentities((string) app('request')->domain()); ?>/<?php echo htmlentities((string) app('request')->url()); ?>
					</a>
				</div>
			</section>
				...
				<ul class="jieda" id="jieda">
					...
					<li data-id="<?php echo htmlentities((string) $comment['id']); ?>" class="jieda-daan">
						<div class="detail-about detail-about-reply">
							<a class="fly-avatar" href="<?php echo url("user/home",["id"=>$comment['user_id']])->domain(true); ?>">
								<img src="<?php echo htmlentities((string) $comment['user']['user_img']); ?>" alt="<?php echo !empty($comment['user']['nickname']) ? htmlentities((string) $comment['user']['nickname']) : htmlentities((string) $comment['user']['name']); ?>">
							</a>
							<div class="fly-detail-user">
								<a href="<?php echo url("user/home",["id"=>$comment['user_id']])->domain(true); ?>" class="fly-link">
								<cite><?php echo !empty($comment['user']['nickname']) ? htmlentities((string) $comment['user']['nickname']) : htmlentities((string) $comment['user']['name']); ?></cite>
								</a>
								<?php if($article['user_id'] == $comment['user_id']): ?><span>(<?php echo lang('poster'); ?>)</span><?php endif; ?>
								<span><?php echo $comment['user']['sign']; ?></span>
							</div>
							<div class="detail-hits">
								<span class="post-time" data="<?php echo htmlentities((string) $comment['create_time']); ?>"></span><?php echo hook('ipShow',$comment['user']['city']); ?></span>
								<?php if($comment['cai'] == 1): ?><i class="iconfont icon-caina" title="最佳答案"></i><?php endif; ?>
							</div>
							...
							<?php if(($article['read_type'] == 0 || (($article['read_type'] == 1) && $passJieMi))): ?>
								...
								<div class="jieda-reply">
									...
									<?php endif; if(($comment['cai'] == 0) && ((session('user_id') == $article['user_id']) OR ($user['auth'] ?? ''))): ?>
										<span class="jieda-accept" type="accept"><?php echo lang('accept'); ?></span>
									<?php endif; ?>
									</div>
										...
										<div class="layui-clear" style="margin:10px 0; padding: 10px; border: 1px solid #f0f0f0; background: #f6f6f6">
											...
											<div style="float: left;width: calc(100% - 50px);">
												<div>
													...
													- <?php echo htmlentities((string) $vo['user']['name']); ?> - <?php echo htmlentities((string) date('Y-m-d H:i',!is_numeric($vo['create_time'])? strtotime($vo['create_time']) : $vo['create_time'])); ?>
												</div>
										...
										<div class="layui-clear" style="margin:10px 0; padding: 10px; border: 1px solid #f0f0f0;">
											<div style="float: left;width: calc(100% - 50px);">
												<div>
													...
													<?php echo htmlentities((string) $voo['user']['name']); ?> @ <?php echo htmlentities((string) $voo['touser']); ?> <?php echo htmlentities((string) date('Y-m-d H:i',!is_numeric($voo['create_time'])? strtotime($voo['create_time']) : $voo['create_time'])); ?>
												</div>
			...
			<div class="layui-form layui-form-pane">
				...
				<div class="layui-input-block">
					<textarea id="L_content"  name="content" required lay-verify="required" placeholder="<?php echo lang('please input the content'); ?>" class="layui-textarea taonyeditor" style="height: 150px"></textarea>
				</div>
				</div>
				<div class="layui-form-item que-comments">
				<input type="hidden" name="article_id" value="<?php echo htmlentities((string) $article['id']); ?>" />
				<button class="layui-btn layui-btn-danger" lay-filter="user-comment" lay-submit><?php echo lang('submit comments'); ?></button>
				</div>
			</div>
			<?php endif; ?>
		</div>
		...
		<div class="layui-col-md4">
			<div class="fly-panel">
				...
				<div class="questions">
					<span class="layui-badge layui-bg-green">回答 <?php echo htmlentities((string) $article['user']['comments_count']); ?></span> <span class="layui-badge layui-bg-green">提问 <?php echo htmlentities((string) $article['user']['article_count']); ?></span>
...
<script>
  ...
  var collectionFind = "<?php echo url('Collection/find'); ?>",
    ...
    laypage.render({
		...
		count: "<?php echo htmlentities((string) $article['comments_count']); ?>", //数据总数，从服务端得到
		limit: 10,
