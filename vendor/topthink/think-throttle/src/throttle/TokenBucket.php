...
class TokenBucket extends ThrottleAbstract
{
    ...
    public function allowRequest(string $key, float $micro_now, int $max_requests, int $duration, CacheInterface $cache): bool
    {
        ...
        // 部分情况缓存返回 false ，需防御处理
        $last_time = (float)($cache->get($key, 0) ?: 0);
        $store_num = (int)($cache->get($assist_key, 0) ?: 0);
        ...
        if ($token_left < 1) {
            ...
            $elapsed = intval(($micro_now - $last_time)) % $tmp;
            $this->wait_seconds = max(1, $tmp - $elapsed);
            return false;
        }
