...
class Service extends \think\Service
{
    ...
    public function boot()
    {
        ...
        // 注册插件控制器路由
        // $route->rule("addons/:addon/[:controller]/[:action]", $execute)->middleware(Addons::class); // 原始
        $route->rule("app/:addon/[:controller]/[:action]", $execute)->middleware(Addons::class);
                    ...
                    foreach ($files as $file) {
                        if (file_exists($file)) {
                            if (substr($file, -11) == 'console.php') {
                                ...
                                if (isset($commands_config['commands']) && is_array($commands_config['commands'])) {
