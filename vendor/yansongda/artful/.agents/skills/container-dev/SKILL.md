---
name: container-dev
description: "Use when local PHP environment is unavailable. Fallback container-based development environment for yansongda/artful project."
---
...
| Web 文档预览 | `pnpm web:preview`（需本地 Node 或 Node 容器） |
...
Web 文档使用 VitePress，镜像中无 Node.js，需使用 Node 镜像或本地运行。

**注意**：`web/` 使用 VitePress 1.x + Vite 6，要求 Node `>=18`，建议使用 `node:20-alpine`。
...
- `composer.lock` 不纳入版本管理（`.gitignore`），容器内 `composer install` 会按 `composer.json` 约束解析最新依赖
